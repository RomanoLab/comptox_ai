      ...
      <TableRow>
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ margin: 1 }}>
              <Button variant="outlined" onClick={handleNodeSearch}>
                Send to Node Search
              </Button>
            </Box>
          </Collapse>
...
const StructureSearchControls = (props) => {
  const [currentMolData, setCurrentMolData] = useReducer(
    setCurrentMolReducer,
    ''
  );
    ...
    <div className="chemical-search">
      ...
      >
        {error ? (
          ...
          <>
            <StructureResultsTable data={data} />
          </>
        ) : null}
      </Box>
    </div>
...
const ChemicalSearch = (props) => {
  return (
    <div className="chemical-search subject-container">
      <h2>Chemical Search</h2>
      <p>
        <i>Find chemicals by drawing a molecular structure.</i>
      </p>
      <StructureEditor />
